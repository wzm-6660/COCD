...
'''
    def next_batch(self):
        ...
        for count in range(self.batch_size):
            log = self.data[self.ptr + count]
            knowledge_emb = [0.] * self.knowledge_dim
            # knowledge_code= log['knowledge_code'][0]
            for knowledge_code in log['knowledge_code']:
                knowledge_emb[knowledge_code - 0] = 1.0

            knowledge_emb[knowledge_code - 0] = 1.0
            y = log['score']
            input_stu_ids.append(log['user_id'] - 0)
            input_exer_ids.append(log['exer_id'] - 0)
            input_knowedge_embs.append(knowledge_emb)
            ys.append(y)
...
class ValTestDataLoader(object):
    def __init__(self, d_type='predict'):
        ...
        if d_type == 'test':
            data_file = '../data/EdNet-1/test_set.json'
        else:
            data_file='../data/EdNet-1/untestdata.json'
        config_file = './config.txt'
        with open(data_file, encoding='utf8') as i_f:
            self.data = json.load(i_f)
        with open(config_file) as i_f:
    ...
    def next_batch(self):
        ...
        for log in logs:
            input_stu_ids.append(user_id - 0)
            input_exer_ids.append(log['exer_id'] -0)
            knowledge_emb = [0.] * self.knowledge_dim
            # knowledge_code = log['knowledge_code'][0]
            # knowledge_emb[knowledge_code - 1] = 1.0
    ...
    def load_all_data(self):
        ...
        for item in self.data:
            ...
            for log in logs:
                ...
                input_exer_ids.append(log['exer_id'] - 1-int(self.knowledge_dim))
        ...
        input_stu_ids = torch.tensor(input_stu_ids, dtype=torch.int64)
        input_exer_ids = torch.tensor(input_exer_ids, dtype=torch.int64)
        input_knowledge_embs = torch.tensor(input_knowledge_embs, dtype=torch.float32)
        ys = torch.tensor(ys, dtype=torch.float32)

        dataset = TensorDataset(input_stu_ids, input_exer_ids, input_knowledge_embs, ys)
